    ...
    <div className="px-4 pt-6 pb-8 animate-fade-in md:px-6 lg:px-8 lg:max-w-3xl">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div className="flex-1">
          <h1 className="text-lg font-bold text-slate-900">{t("detail.title")}</h1>
        </div>
      ...
      <div className="bg-white/90 backdrop-blur-sm rounded-3xl overflow-hidden shadow-sm border border-white/60 mb-4">
        {imgSrc && (
          <div className="w-full h-60 bg-slate-100 relative">
            <img
              src={imgSrc}
        ...
        <div className="p-5">
          <h2 className="text-xl font-bold text-slate-900">
            {request.productName || "Product Image"}
          </h2>
          {request.description && (
            <p className="text-sm text-slate-500 mt-1.5 leading-relaxed">{request.description}</p>
          )}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-emerald-600 mt-0.5">
                {formatFullDate(request.order.createdAt)}
              </p>
            </div>
          </div>
        </div>
          ...
          {showPriceInput ? (
            <div className="bg-white/90 backdrop-blur-sm rounded-2xl border border-white/60 p-5 shadow-sm">
              ...
              <div className="flex gap-2">
                ...
                <button
                  onClick={() =>
                    handleAction(() => setPrice(id, parseFloat(priceInput)))
                  }
              ...
              <button
                ...
                className="text-sm text-slate-400 mt-3 font-medium"
              >
                {t("detail.cancel")}
              </button>
            </div>
            ...
            <>
              ...
              <button
                onClick={() => handleAction(() => markUnavailable(id))}
      ...
      {isBuyer && request.status === "PRICED" && (
        <div className="space-y-3 animate-slide-up">
          <button
            onClick={() => handleAction(() => acceptPrice(id))}
            disabled={actionLoading}
          ...
          <button
            onClick={() => handleAction(() => rejectPrice(id))}
            disabled={actionLoading}
